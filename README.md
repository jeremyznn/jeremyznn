<p align="center">
  <img
    src="https://capsule-render.vercel.app/api?type=waving&height=230&color=0%3A020617%2C45%3A0F172A%2C100%3A10B981&text=JEREMY&fontAlign=50&fontAlignY=38&fontColor=FFFFFF&fontSize=68&desc=Developpeur+web+full-stack+%7C+React+%2F+Next.js+%2F+Node.js&descAlign=50&descAlignY=58&descSize=18&animation=fadeIn"
    alt="Jeremy"
  />
</p>

<p align="center">
  <a href="https://zenoxhq.fr">
    <img src="https://img.shields.io/badge/Portfolio-zenoxhq.fr-10B981?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
  </a>
  <a href="https://github.com/jeremyznn?tab=repositories">
    <img src="https://img.shields.io/badge/Repos-GitHub-111827?style=for-the-badge&logo=github&logoColor=white" alt="Repos GitHub" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=jeremyznn&style=for-the-badge&color=10B981&label=VISITES" alt="Vues du profil" />
</p>

<p align="center">
  <img
    src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=24&duration=2600&pause=900&color=10B981&center=true&vCenter=true&width=900&lines=Developpeur+web+full-stack;React+%2F+Next.js+%2F+Node.js;Interfaces+modernes+et+outils+web;Dashboards%2C+APIs+et+projets+propres"
    alt="Présentation animée"
  />
</p>

<br />

## À propos

Moi c’est **Jérémy**, aussi connu sous le pseudo **ZENOX**.

Je développe surtout des interfaces web avec **React**, **Next.js** et **Tailwind CSS**.
J’aime quand un projet est propre, clair, fluide et agréable à utiliser. Pas juste un truc joli en surface, mais quelque chose qui reste bien organisé dans le code.

Je touche aussi au back-end avec **Node.js**, **MongoDB** et **MySQL**, principalement pour créer des APIs, gérer des données et connecter les interfaces à de vraies fonctionnalités.

Je continue de progresser projet après projet, en essayant de garder une base simple : faire propre, utile, responsive et compréhensible.

<br />

```js
const jeremy = {
  alias: "ZENOX",
  role: "Développeur web full-stack",
  website: "https://zenoxhq.fr",
  stack: {
    front: ["React", "Next.js", "Tailwind CSS"],
    back: ["Node.js", "MongoDB", "MySQL"],
    tools: ["Git", "GitHub", "Vercel"],
  },
  focus: [
    "interfaces modernes",
    "dashboards",
    "APIs",
    "outils web",
    "projets propres",
  ],
};
```

<br />

## Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=js,react,nextjs,nodejs,mongodb,mysql,html,css,tailwind,git,github,vercel&theme=dark" alt="Stack technique" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/JavaScript-111827?style=for-the-badge&logo=javascript&logoColor=F7DF1E" alt="JavaScript" />
  <img src="https://img.shields.io/badge/React-111827?style=for-the-badge&logo=react&logoColor=61DAFB" alt="React" />
  <img src="https://img.shields.io/badge/Next.js-020617?style=for-the-badge&logo=nextdotjs&logoColor=white" alt="Next.js" />
  <img src="https://img.shields.io/badge/Node.js-0F172A?style=for-the-badge&logo=nodedotjs&logoColor=5FA04E" alt="Node.js" />
  <img src="https://img.shields.io/badge/MongoDB-052E16?style=for-the-badge&logo=mongodb&logoColor=4EA94B" alt="MongoDB" />
  <img src="https://img.shields.io/badge/MySQL-111827?style=for-the-badge&logo=mysql&logoColor=4479A1" alt="MySQL" />
  <img src="https://img.shields.io/badge/Tailwind%20CSS-0F172A?style=for-the-badge&logo=tailwindcss&logoColor=38BDF8" alt="Tailwind CSS" />
  <img src="https://img.shields.io/badge/Git-111827?style=for-the-badge&logo=git&logoColor=F05032" alt="Git" />
</p>

<br />

## Ce que j’aime construire

<table>
  <tr>
    <td width="50%">
      <h3>Interfaces web</h3>
      <p>
        Des interfaces propres, modernes et responsive avec React, Next.js et Tailwind CSS.
      </p>
    </td>
    <td width="50%">
      <h3>Dashboards</h3>
      <p>
        Des espaces clairs pour afficher, organiser et gérer des données sans rendre l’expérience lourde.
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3>APIs</h3>
      <p>
        Des routes API, de la logique serveur et des connexions avec des bases de données.
      </p>
    </td>
    <td width="50%">
      <h3>Outils web</h3>
      <p>
        Des outils pratiques, des pages admin et des projets perso pour tester, apprendre et progresser.
      </p>
    </td>
  </tr>
</table>

<br />

## Projets

<table>
  <tr>
    <td width="50%">
      <h3>Portfolio</h3>
      <p>
        Mon site personnel, avec mes projets, mon profil et mon évolution dans le développement web.
      </p>
      <a href="https://zenoxhq.fr">
        <img src="https://img.shields.io/badge/Ouvrir%20le%20portfolio-10B981?style=for-the-badge&logo=vercel&logoColor=white" alt="Ouvrir le portfolio" />
      </a>
    </td>
    <td width="50%">
      <h3>Repos GitHub</h3>
      <p>
        Une partie de mes projets, tests et expérimentations autour du web, du front-end et du back-end.
      </p>
      <a href="https://github.com/jeremyznn?tab=repositories">
        <img src="https://img.shields.io/badge/Voir%20les%20repos-111827?style=for-the-badge&logo=github&logoColor=white" alt="Voir les repos" />
      </a>
    </td>
  </tr>
</table>

<br />

## En ce moment

<table>
  <tr>
    <td width="50%">
      <h3>Front-end</h3>
      <p>
        Je travaille sur des interfaces plus propres, avec une meilleure structure, de meilleurs composants et un rendu plus professionnel.
      </p>
    </td>
    <td width="50%">
      <h3>Full-stack</h3>
      <p>
        Je développe aussi des fonctionnalités côté serveur, avec des APIs, des bases de données et des intégrations utiles.
      </p>
    </td>
  </tr>
</table>

<br />

## Activité GitHub

<p align="center">
  <img
    src="https://streak-stats.demolab.com?user=jeremyznn&theme=transparent&hide_border=true&ring=10B981&fire=10B981&currStreakLabel=10B981&sideLabels=CBD5E1&currStreakNum=FFFFFF&sideNums=FFFFFF&dates=94A3B8"
    alt="Streak GitHub"
  />
</p>

<p align="center">
  <img
    src="https://github-readme-activity-graph.vercel.app/graph?username=jeremyznn&bg_color=020617&color=CBD5E1&title_color=10B981&line=10B981&point=FFFFFF&area=true&hide_border=true&radius=12&custom_title=Activite%20recente"
    alt="Graphique d'activité GitHub"
  />
</p>

<br />

<p align="center">
  <strong>Des interfaces propres, modernes et utiles.</strong>
</p>

<p align="center">
  <img
    src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0%3A10B981%2C45%3A0F172A%2C100%3A020617"
    alt="Footer"
  />
</p>
